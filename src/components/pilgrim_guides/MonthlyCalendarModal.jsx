...
export default function MonthlyCalendarModal({
	...
	const [selectedTimeSlot, setSelectedTimeSlot] = useState(null); // Selected time slot (e.g., "10:00-11:00")
	...
	// Get all unique time slots available across all dates
	const getUniqueTimeSlots = () => {
		const timeSlotMap = new Map();
		
		localSlots.forEach((slot) => {
			const slotType = (slot.type || "individual").toLowerCase();
			const selectedOccupancy = dynamicOccupancyType.toLowerCase();
			
			// Only include slots matching current occupancy type
			if (slotType === selectedOccupancy && isSlotVisible(slot)) {
				const timeKey = `${slot.time}-${slot.endTime}`;
	...
	// Get dates that have a specific time slot available
	const getDatesForTimeSlot = (timeSlot) => {
		if (!timeSlot) return [];
		...
		return localSlots
			.filter((slot) => {
				const slotType = (slot.type || "individual").toLowerCase();
				const selectedOccupancy = dynamicOccupancyType.toLowerCase();
				const timeMatches = slot.time === timeSlot.startTime && slot.endTime === timeSlot.endTime;
				const typeMatches = slotType === selectedOccupancy;
				const isVisible = isSlotVisible(slot);
				const notAlreadyBooked = !hasSlotOnDate(slot.date);
				const notAlreadySelected = !selectedSlotsMulti.some(s => s.date === slot.date);
				
				return timeMatches && typeMatches && isVisible && notAlreadyBooked && notAlreadySelected;
			})
			.map((slot) => slot.date)
			.sort();
	};
	...
	const handleTimeSlotSelect = (timeSlot) => {
		...
		setSelectedDate(null); // Reset date selection when changing time slot
	};

	// Handle date selection for the selected time slot
	const handleDateSelectForTimeSlot = (dateStr) => {
		...
		const matchingSlot = localSlots.find(
			(slot) =>
				slot.date === dateStr &&
				slot.time === selectedTimeSlot.startTime &&
				slot.endTime === selectedTimeSlot.endTime &&
				isSlotVisible(slot)
		);

		if (matchingSlot) {
			setSelectedSlotsMulti((prev) => [...prev, matchingSlot]);
		}
	};
	...
	const handleAddMonthlyToCart = async () => {
		try {
			...
			// Primary path: sessionData?.[modeKey]?.monthly?.discount
			const discountFromPlan = planData?.discount ?? null;
			const discountFromMode = sessionData?.[modeKey]?.monthly?.discount ?? null;
			const discountFromSlides = sessionData?.slides?.[0]?.[modeKey]?.monthly?.discount ?? 
										sessionData?.slides?.[modeKey]?.monthly?.discount ?? null;
			
			// Priority: direct mode path > plan > slides
			const monthlyDiscountPercent = discountFromMode ?? discountFromPlan ?? discountFromSlides;
				...
				<div className="p-6">
					{loading ? (
						...
						<>
							...
							{/* Monthly View - New flow: Select time slot first, then dates */}
							{dynamicOccupancyType.toLowerCase() === "group" ? (
								// Original calendar view for group bookings
								<div>
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
										...
										{/* Right-side slot panel or Info for Group */}
										{selectedDate ? (
															...
															return (
																...
																>
																	<div>
																		...
																		<p
																			className={`text-xs mt-1 ${isGroupMode
																					? "text-blue-700"
																					: isCoupleMode
																						? capacityInfo.isFull
																							? "text-red-600"
								...
								// New flow for Individual and Couple: Select time slot first, then dates
								<div className="space-y-6">
									{/* Step 1: Select Time Slot */}
									<div>
										<h4 className="font-medium text-gray-800 mb-3">
											Step 1: Choose Your Preferred Time Slot
										</h4>
										<p className="text-sm text-gray-600 mb-4">
											Select a time slot, then pick dates that work for you
										</p>
										<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
											{getUniqueTimeSlots().map((timeSlot, idx) => {
												const isSelected = selectedTimeSlot?.startTime === timeSlot.startTime && 
																  selectedTimeSlot?.endTime === timeSlot.endTime;
												const availableDatesCount = getDatesForTimeSlot(timeSlot).length;
												...
												return (
													<button
														...
														className={`p-4 rounded-lg border-2 transition-all text-left ${
															isSelected
																? "border-[#2F6288] bg-blue-50"
																: "border-gray-300 bg-white hover:border-[#2F6288] hover:bg-gray-50"
														}`}
													>
														<div className="flex justify-between items-center">
															...
															<div className="text-right">
																<p className="text-xs text-gray-500">
																	{availableDatesCount} date{availableDatesCount !== 1 ? 's' : ''} available
																</p>
																{isSelected && (
										...
										{getUniqueTimeSlots().length === 0 && (
											<p className="text-sm text-gray-500 text-center py-4">
												No time slots available for {dynamicOccupancyType} booking
											</p>
										)}
									</div>

									{/* Step 2: Select Dates (only shown after time slot is selected) */}
									{selectedTimeSlot && (
										<div>
											<div className="flex items-center justify-between mb-3">
												<h4 className="font-medium text-gray-800">
													Step 2: Choose Dates for {selectedTimeSlot.startTime} - {selectedTimeSlot.endTime}
												</h4>
												<button
													onClick={() => {
														setSelectedTimeSlot(null);
											...
											<p className="text-sm text-gray-600 mb-4">
												Click on dates to add them to your selection. You can select up to {remainingSessionsAfterCart} more session(s).
											</p>
											...
											<div className="grid grid-cols-7 gap-1">
												{getDaysInMonth(currentDate).map((day, index) => {
													...
													const availableDates = getDatesForTimeSlot(selectedTimeSlot);
													const isAvailable = availableDates.includes(dateStr);
													const hasUserSlot = hasSlotOnDate(dateStr);
													const isAlreadySelected = selectedSlotsMulti.some(s => s.date === dateStr);

													return (
														<button
															key={index}
															onClick={() => isAvailable && !isAlreadySelected && handleDateSelectForTimeSlot(dateStr)}
															disabled={!isAvailable || hasUserSlot || isAlreadySelected}
															className={`p-2 text-center text-sm rounded transition-colors ${
																hasUserSlot
																	...
																	: isAlreadySelected
																		...
																		: isAvailable
																			? "bg-blue-100 text-blue-800 hover:bg-blue-200 cursor-pointer font-medium"
																			: "text-gray-300 cursor-not-allowed"
															}`}
														>
															{day}
											...
											{getDatesForTimeSlot(selectedTimeSlot).length === 0 && (
												<div className="mt-4 text-center">
													<p className="text-sm text-gray-500">
														No available dates for this time slot this month
													</p>
													<button
														onClick={() => setSelectedTimeSlot(null)}
														className="mt-2 text-sm text-[#2F6288] hover:underline"
													>
