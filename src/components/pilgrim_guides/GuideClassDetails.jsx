                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {currentOccupancies.map((occupancy, index) => (
                                            <button
                                                ...
                                                className={`px-4 py-2 rounded-lg border transition-all duration-200 text-sm ${selectedOccupancy?.type === occupancy.type
                        ...
                        <div className="max-w-sm">
                            ...
                            {subscriptionType &&
                                (() => {
                                    ...
                                    // Get discount info for monthly plans
                                    let discount = null;
                                    let description = null;
                                    let hasValidDiscount = false;
                                    let originalPrice = null;
                                    
                                    if (subscriptionType === "monthly" && sessionData && mode) {
                                        const modeKey = mode.toLowerCase();
                                        const monthlyData = sessionData?.[modeKey]?.monthly || {};
                                        discount = monthlyData?.discount;
                                        description = monthlyData?.description;
                                        const discountNum = Number(discount);
                                        hasValidDiscount = Number.isFinite(discountNum) && discountNum > 0;
                                        
                                        if (hasValidDiscount) {
                                            // price is the original price, calculate discounted price
                                            originalPrice = price;
                                        }
                                    }
                                    
                                    // Calculate discounted price if there's a valid discount
                                    const discountedPrice = hasValidDiscount && originalPrice 
                                        ? Math.round(originalPrice * (1 - Number(discount) / 100))
                                        : null;
                                    ...
                                    return (
                                        <div className="mt-2 border rounded-xl p-4 bg-white">
                                            ...
                                            {hasValidDiscount && originalPrice && discountedPrice ? (
                                                <>
                                                    ...
                                                    <div className="text-sm font-medium" style={{ color: 'rgb(47, 98, 136)' }}>
                                                        {discount}% off
                                                    </div>
                                                </>
                                            ) : (
                                            ...
                                            {description && subscriptionType === "monthly" && (
                                                <div
                                                    className="text-xs mt-3 pt-3 border-t prose-sm max-w-none text-gray-700"
